...
export default function Crud({ params }: { params: { crud: string[] } }) {
  ...
  const { data: hireMeData } = useQuery({
    ...
    queryFn: async () => GetRequestById(Number(hireMeId)),
  });
  ...
  if (params.crud[0] === PageAction.view) {
    return (
      <div className="flex justify-center items-center w-full ">
        <div className="w-5/12 ">
          <NewHireMeForm data={hireMeData!} pageAction={PageAction.view} />
        </div>
      </div>
    );
    ...
    return (
      <div className="flex justify-center items-center w-full ">
        <div className="w-5/12 ">
          <NewHireMeForm data={hireMeData!} pageAction={PageAction.delete} />
        </div>
      </div>
    );
  }
